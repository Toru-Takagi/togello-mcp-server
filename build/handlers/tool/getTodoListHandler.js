...
export const getTodoListHandler = async ({ categoryUUIDs, completionStatus, completedStartDate, completedEndDate, }) => {
    try {
        ...
        if (hasCompletedStartDate !== hasCompletedEndDate) {
            return errorToolResponse('completedStartDate and completedEndDate must be specified together');
        }
        if ((hasCompletedStartDate || hasCompletedEndDate) &&
            completionStatus !== 'COMPLETED') {
            return errorToolResponse('completionStatus must be COMPLETED when completed date filters are specified');
        }
